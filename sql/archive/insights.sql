...
WITH employment_growth AS (
    ...
    GROUP BY region_id, industry_code, SUBSTRING(year_month, 1, 4)
),
yoy_growth AS (
    SELECT
        curr.region_id,
...
-- [인사이트 2] 실업률 변동성이 큰 지역 (고용 불안정성)
WITH monthly_stats AS (
    SELECT
        region_id,
        AVG(unemployment_rate) as avg_rate,
        STDDEV(unemployment_rate) as std_rate,
...
industry_concentration AS (
    SELECT
        s.region_id,
        SUM(POWER(s.avg_employed / t.total_employed, 2)) as hhi
...
SELECT
    ...
    ROUND((post.avg_employed_2024 - pre.avg_employed_2019) / pre.avg_employed_2019 * 100, 2) as recovery_rate_pct,
    CASE
        WHEN (post.avg_employed_2024 - pre.avg_employed_2019) / pre.avg_employed_2019 > 0.05 THEN '강한 회복'
        WHEN (post.avg_employed_2024 - pre.avg_employed_2019) / pre.avg_employed_2019 > 0 THEN '회복'
        ELSE '미회복'
...
WITH yearly_participation AS (
    SELECT
        u.region_id,
        CAST(SUBSTRING(u.year_month, 1, 4) AS UNSIGNED) as year,
        AVG(u.labor_force / p.total_pop * 100) as participation_rate
    ...
    GROUP BY u.region_id, CAST(SUBSTRING(u.year_month, 1, 4) AS UNSIGNED)
)
SELECT
    r.region_name,
